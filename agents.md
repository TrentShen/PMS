...
## 12. 反过度设计约束（2026-08 新增）

> 来源：《防止 AI 产出高质量代码时陷入过度设计的深度研究报告》。PMS 是 100 人内小团队的内部系统，复杂度预算必须与之匹配。

### 12.1 默认姿势

- **只做当前验收标准要求的**，不为假设中的未来需求加抽象（YAGNI，默认"有罪"）
- **抽象后置**：相似代码稳定出现 3 处以上且差异集中，才允许抽公共层/组件；否则保持局部重复
- **变更预算**：单功能新增文件 ≤3、修改文件 ≤10、新增依赖 = 0（确需新依赖必须先说明理由并获确认）
- **优先贴现有模式**：复用项目里已有的组件/写法，不发明新层；宁可局部重复，不提前通用化
- **审查看健康不追求完美**：改动明确改善了系统就通过，不用"还可以再架构一下"无限加码

### 12.2 禁止清单（没有明确证据不做）

- 不为小需求引入：插件机制、策略模式、事件总线、消息队列、统一缓存层、配置平台、feature flag 系统
- 不把"生产级"理解为平台化：本项目的最低必要集合 = 权限校验 + 审计日志 + 健康检查 + 可回滚迁移
- 不重构与本任务无关的代码；不顺手"优化"周围结构
- 不为了"以后可能开放"提前暴露公开扩展点

### 12.3 任务开工前自检（Kimi 每次执行前默答）

1. 验收标准明确吗？（输入/输出/失败条件）
2. 用户明确说了"不需要做什么"吗？没有的话，涉及新增抽象/依赖/表结构时先确认
3. 这个功能 100 人团队真的需要吗？有"大公司味"的设计先降级
4. 完成时说明**刻意没做什么**（Rejected over-engineering options）
